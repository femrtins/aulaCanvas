# utils/select_tool.py (Versão Enxuta)

from utils.glut_utils import *
# Assumindo que 'Tool' está em 'input/tool.py'
...
class SelectTool(Tool):
    ...
    def activate(self):
        self.mode = 'translate'
        print("Ferramenta de Seleção Ativa. Modos: [t]ransladar, [e]scalar, [o] rotacionar")
    ...
    def select_geometry(self, x, y):
        ...
        for geometry in reversed(self.canvas.geometries):
            # Assumindo que geometry.contains_point() já lida com rotação
            # (como o que fizemos para o Retângulo)
    ...
    def _translate(self, current_x, current_y):
        ...
        # E move todos os pontos da polilinha 
        if hasattr(g, 'points'):
            g.points = [(px + dx, py + dy) for (px, py) in g.points]

    def _scale(self, current_x, current_y):
        """Escala o objeto com base no movimento do mouse."""
        if self.start_x is None: return
        
        g = self.selected_geometry
        
        # Calcula um fator de escala simples
        # (Movimento para a direita/cima aumenta, esquerda/baixo diminui)
        dx = current_x - self.start_x
        dy = current_y - self.start_y
        
        scale_factor = 1.0 + (dx + dy) * 0.01 # Fator de sensibilidade
        scale_factor = max(0.1, scale_factor) # Evita inverter
        ...
        if hasattr(g, 'width'):
            ...
            # Escala polilinha em relação ao seu pivô (x0, y0)
            new_points = []
    ...
    def _rotate(self, current_x, current_y):
        ...
        dy = current_y - self.start_y
        angle_delta = dy * 1.0 # Arrastar 1 unidade = 1 grau
        
        self.selected_geometry.angle += angle_delta


# -----------------------------------------------------------------
# CLASSE DA CAIXA DE SELEÇÃO (Enxuta)
# -----------------------------------------------------------------
class SelectionBox:
    ...
    def draw(self):
        ...
        try:
            ...
            if g.type == "Rec":
                # 1. Encontra o pivô e as dimensões locais
                cx = g.x0 + g.width / 2.0
                cy = g.y0 + g.height / 2.0
                w_half = g.width / 2.0
                h_half = g.height / 2.0
                
                # 2. Aplica as transformações do objeto
    ...
    def _draw_box_outline(self, x_min, y_min, x_max, y_max):
        """Função helper para desenhar a caixa e os handles."""
        
        glLineStipple(5, 0xAAAA)
        glEnable(GL_LINE_STIPPLE)
        glColor3f(0.2, 0.2, 0.8) # Azul escuro
        glLineWidth(2.0)
    ...
    def _draw_handle(self, x, y, size=1):
        """Desenha um pequeno quadrado centrado em (x, y)."""
        s = size 
        glBegin(GL_QUADS)
        glVertex2f(x - s, y - s)
        glVertex2f(x + s, y - s)
